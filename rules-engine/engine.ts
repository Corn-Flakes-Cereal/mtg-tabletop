...
export type CombatStep = 'begin' | 'declare_attackers' | 'declare_blockers' | 'combat_damage' | 'end' | null;
...
export const TURN_STRUCTURE: readonly StepDef[] = [
...
export interface RoomState {
  ...
  passed: PlayerId[]; // players who have passed in succession since the stack last changed or the step began
  stack: StackObject[]; // top of stack = last element
  log: LogEntry[];
}
...
function log(state: RoomState, text: string): void {
  state.log.push({ ts: Date.now(), text });
  if (state.log.length > 300) state.log.shift();
}

function playerName(state: RoomState, id: PlayerId | null | undefined): string {
  if (!id) return '?';
  return state.players[id]?.name ?? '?';
}

export function addPlayer(state: RoomState, playerId: PlayerId, name: string): void {
...
export function setDisconnected(state: RoomState, playerId: PlayerId): void {
  const existing = state.players[playerId];
  if (existing) {
...
function currentStepDef(state: RoomState): StepDef {
  const def = TURN_STRUCTURE[state.structureIndex];
  if (!def) throw new Error(`Invariant violated: structureIndex ${state.structureIndex} out of range`);
  return def;
}
...
export function activePlayerId(state: RoomState): PlayerId {
  const id = state.playerOrder[state.activePlayerIndex];
  if (!id) throw new Error('Invariant violated: no active player (has the game started?)');
  return id;
}

function enterStep(state: RoomState): void {
...
function advanceStep(state: RoomState): void {
  state.structureIndex++;
  if (state.structureIndex >= TURN_STRUCTURE.length) {
...
export function startGame(state: RoomState): ActionResult {
  if (state.started) return { ok: false, error: 'Game already started.' };
  if (state.playerOrder.length < 1) return { ok: false, error: 'Need at least 1 player to start.' };
  state.started = true;
  state.turnNumber = 1;
...
export function passPriority(state: RoomState, playerId: PlayerId): ActionResult {
  if (!state.started) return { ok: false, error: 'Game has not started yet.' };
  if (state.priorityPlayerId !== playerId) return { ok: false, error: "It's not your priority." };
  ...
  if (!nextPlayer) throw new Error('Invariant violated: empty playerOrder during priority pass');
...
export function addToStack(state: RoomState, playerId: PlayerId, label: string): ActionResult {
  if (!state.started) return { ok: false, error: 'Game has not started yet.' };
  if (state.priorityPlayerId !== playerId) return { ok: false, error: "It's not your priority." };
  const trimmed = (label || '').trim();
  if (!trimmed) return { ok: false, error: 'Give the stack object a label.' };

  const obj: StackObject = { id: nanoid(), controllerId: playerId, label: trimmed };
...
export function viewFor(state: RoomState): ViewState {
  const stepDef = currentStepDef(state);
  return {
    code: state.code,
